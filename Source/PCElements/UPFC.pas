...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
   TUPFC = CLASS(TPCClass)
     private
       Procedure UPFCSetBus1(const S:String);
     Protected
...
Begin
     ...
     PropertyHelp[20]  := 'Positive-sequence equivalent source impedance, ohms, as a 2-element array representing a complex number. Example: '+CRLF+CRLF+
                          ...
                          'If defined, Z1, Z2, and Z0 are used to define the impedance matrix of the UPFC. ' +
     ...
     PropertyHelp[21]  := 'Zero-sequence equivalent source impedance, ohms, as a 2-element array representing a complex number. Example: '+CRLF+CRLF+
                          ...
                          'Used to define the impedance matrix of the UPFC if Z1 is also specified. '+CRLF+CRLF+
     ...
     PropertyHelp[23]  := '2-element array: e.g., [1  2]. An alternate way to specify Z1. See Z1 property. Per-unit positive-sequence impedance on base of UPFC BasekV and BaseMVA.';
     PropertyHelp[24]  := '2-element array: e.g., [1  2]. An alternate way to specify Z0. See Z0 property. Per-unit zero-sequence impedance on base of UPFC BasekV and BaseMVA.';
     PropertyHelp[25]  := '2-element array: e.g., [1  2]. An alternate way to specify Z2. See Z2 property. Per-unit negative-sequence impedance on base of UPFC BasekV and BaseMVA.';
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
procedure TUPFC.UPFCSetBus1(const S: String);
Var
   s2:String;
   i, dotpos:Integer;
  ...
  WITH ActiveUPFCObj DO Begin
     ...
     WHILE Length(Param) > 0 DO Begin
         ...
         CASE ParamPointer OF
            ...
            1: UPFCSetBus1(param);   // special handling of Bus 1
...
Begin
   ...
   WITH ActiveUPFCObj DO Begin
       ...
       VMag      := OtherUPFC.Vmag;
       kVBase    := OtherUPFC.kVBase;
       BaseMVA   := OtherUPFC.BaseMVA;
       PerUnit   := OtherUPFC.PerUnit;
       Angle     := OtherUPFC.Angle;
       MVAsc3    := OtherUPFC.MVAsc3;
       MVAsc1    := OtherUPFC.MVAsc1;

       Scantype     := OtherUPFC.Scantype;
       Sequencetype := OtherUPFC.Sequencetype;
       SrcFrequency := OtherUPFC.SrcFrequency;

        ZSpecType      := OtherUPFC.ZSpecType;
        R1             := OtherUPFC.R1;
        X1             := OtherUPFC.X1;
        R2             := OtherUPFC.R2;
        X2             := OtherUPFC.X2;
        ...
        Bus2Defined    := OtherUPFC.Bus2Defined;
        Z1Specified    := OtherUPFC.Z1Specified;
        Z2Specified    := OtherUPFC.Z2Specified;
        Z0Specified    := OtherUPFC.Z0Specified;
        puZ0Specified  := OtherUPFC.puZ0Specified;
        puZ1Specified  := OtherUPFC.puZ1Specified;
        puZ2Specified  := OtherUPFC.puZ2Specified;

        {Loadshape stuff}
        ShapeIsActual  := OtherUPFC.ShapeIsActual;
        DailyShape     := OtherUPFC.DailyShape;
        DailyShapeObj  := OtherUPFC.DailyShapeObj;
        DutyShape      := OtherUPFC.DutyShape;
        DutyShapeObj   := OtherUPFC.DutyShapeObj;
        YearlyShape    := OtherUPFC.YearlyShape;
        YearlyShapeObj := OtherUPFC.YearlyShapeObj;
...
Begin
     ...
     Spectrum := 'defaultUPFC';

     ShapeIsActual := FALSE;
     YearlyShape    := '';
     YearlyShapeObj := nil;  // IF YearlyShapeobj = nil THEN the UPFC alway stays nominal
     DailyShape     := '';
     DailyShapeObj  := nil;  // IF DaillyShapeobj = nil THEN the UPFC alway stays nominal
     DutyShape      := '';
     DutyShapeObj   := nil;  // IF DutyShapeobj = nil THEN the UPFC alway stays nominal
...
Begin
   ...
   IF SpectrumObj=NIL Then Begin
          DoSimpleMsg('Spectrum Object "' + Spectrum + '" for Device UPFC.'+Name+' Not Found.', 324);
   End;
    ...
    IF YearlyShapeObj = Nil THEN
      IF Length(YearlyShape)>0 THEN DoSimpleMsg('WARNING! UPFC Yearly load shape: "'+ YearlyShape +'" Not Found.', 34583);
    IF DailyShapeObj = Nil THEN
      IF Length(DailyShape)>0 THEN DoSimpleMsg('WARNING! UPFC Daily load shape: "'+ DailyShape +'" Not Found.', 34584);
    IF DutyShapeObj = Nil THEN
      IF Length(DutyShape)>0 THEN DoSimpleMsg('WARNING! UPFC Duty load shape: "'+ DutyShape +'" Not Found.', 34585);
...
//=============================================================================
Procedure TUPFCObj.GetVterminalForSource;
...
Begin
  ...
  EXCEPT
      DoSimpleMsg('Error computing Voltages for UPFC.'+Name+'. Check specification. Aborting.', 326);
