...
}
{
  ...
  New InvControl.Name=myname Element=devclass.name terminal=[ 1|2|...] CapacitorList = (gen1  gen2 ...)
...
Begin
     ...
     PropertyHelp[2] := 'Number of the terminal of the circuit element to which the InvControl control is connected. '+
...
Begin
   ...
   WITH ActiveInvControlObj Do Begin
        ...
        ElementName       := OtherInvControl.ElementName;
        ControlledElement := OtherInvControl.ControlledElement;  // Pointer to target circuit element
        MonitoredElement  := OtherInvControl.MonitoredElement;  // Pointer to target circuit element

        ElementTerminal   := OtherInvControl.ElementTerminal;


        For i := 1 to ParentClass.NumProperties Do PropertyValue[i] := OtherInvControl.PropertyValue[i];
...
Function TInvControlObj.MakeGenList:Boolean;
